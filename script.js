...
let cantMeses = 3;
let mesActivo = '';
window.__setMesActivo = (k) => { mesActivo = k; renderPresupuesto(); };
...
function exportarReporte() {
  ...
  ws['!cols'] = [{ wch: 12 }, { wch: 10 }, { wch: 14 }, { wch: 14 }, { wch: 20 }, { wch: 14 }, { wch: 20 }, { wch: 14 }];
...
function renderPresupuesto() {
  ...
  tabsEl.innerHTML = keys.map(k => `<button class="mes-btn ${k===mesActivo?'active':''}" onclick="window.__setMesActivo('${k}')">${mesLabel(k)}</button>`).join('');
